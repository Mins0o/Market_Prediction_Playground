...
def get_ticker_list_ohlcvs(ticker_list, name_list=None):
    all_data=[]
    list_size = len(ticker_list)
    if name_list is not None:
        for ii, ticker in enumerate(ticker_list):
            print(f" {ii:5d}/{list_size} {name_list[ii]}", end = "              \r")
            all_data.append(fetch_ohlcvs(ticker))
    else:
        all_data = [fetch_ohlcvs(ticker) for ticker in tqdm(ticker_list)]
    print("Data scraping complete.                        ")
    return all_data

def get_all_data():
    ticker_list = get_all_tickers()
    name_list = get_ticker_names(ticker_list)
    market_records = get_ticker_list_ohlcvs(ticker_list, name_list)
