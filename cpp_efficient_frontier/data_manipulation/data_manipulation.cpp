...
std::vector<std::pair<size_t, double>> extract(
...
std::vector<double> Data::TrimSecurityByDate(
    const std::vector<double>& full_length_security, time_t start,
    time_t end) const {
  ...
  return std::vector<double>(start_it + start_index,
                             start_it + std::min(end_index + 1, max_index));
}
...
size_t Data::FindIndexBySecurityName(const std::string& security_name) const {
  ...
  if (results.empty()) {
    std::cout << "Search result for \"" << security_name << "\" was not found."
              << std::endl;
...
SecurityColumn Data::GetSecurityByIndex(size_t security_index) const {
  return SecurityColumn({security_names_[security_index],
...
size_t Data::GetSecuritiesCount() const { return column_count_; }

data::DateLine Data::GetDateLine() const { return date_line_; }
}  // namespace data
